    ...
    <div class="page_zone" v-loading="loading">
      ...
      <div class="content">
        ...
        >
          ...
          <el-row :gutter="60">
            <el-col :xs="24" :sm="12" :lg="8" :span="8">
              <el-form-item label="本省是否有队伍出动">
                <el-radio-group v-model="hasCddw" @change="hasCddwHandle">
          ...
          <el-row :gutter="60" v-if="ruleForm.isYdwcd == '1'&&hasCddw">
            <el-col :xs="24" :sm="12" :lg="8" :span="8">
          ...
          <el-row :gutter="60">
            <el-col :xs="24" :sm="12" :lg="8" :span="8">
              <el-form-item label="是否责任区主战">
                <el-radio-group @input="isZrqzz" :value="ruleForm.sfzrqzz">
          ...
          <el-row :gutter="60">
            <el-col :xs="24" :sm="12" :lg="12" :span="12">
              <el-form-item label="增援总队（外省）" prop="zyzd">
                ...
                >
                  <i slot="suffix" class="iconfont icon-icon-test" @click="openAdminSelet"></i>
                  <i slot="suffix" class="el-icon-circle-close closeBtnMar" @click="ruleForm.jqxxKqyDepts = []" v-show="ruleForm.jqxxKqyDepts.length"></i>
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12" :lg="8" :span="8" v-if="ruleForm.jqxxKqyDepts.length&&!hasCddw">
              <el-form-item label="主战队伍">
                <el-checkbox
                    v-for="(item,index) in ruleForm.jqxxKqyDepts"
                    :key="item.deptId"
                    :value="item.hasMain"
                    @input="seleMain(index)"
                  >{{ item.deptName }}
                  </el-checkbox>
              </el-form-item>
            </el-col>
          ...
          <el-row :gutter="60">
            ...
            <el-col :xs="24" :sm="12" :lg="8" :span="8">
              <el-form-item label="单位名称/户主名" prop="dwmcHzm">
                <el-input v-model="ruleForm.dwmcHzm" placeholder="没有请填写无" clearable></el-input>
              </el-form-item>
            </el-col>
...
<script>
  import {getOptions, getJqcdlb, kqyProJqxxAdd} from "@/api/data/entry/jqxx";
  // 责任区大队
  ...
  export default {
    name: "kqyjqlrNew",
    components: { FireSpaceMul, modal, mulModal, mainTeamModal, provModal, confirm,selectAdmin },
    computed:{
      adminTeam(){
          ...
          zyzd:[
            {
              validator: (rule, value, callback) => {
                if (!that.ruleForm.jqxxKqyDepts.length) {
                  return callback(new Error("请选择增援总队！"));
                }
                callback();
              },
              trigger: ["blur","change"],
    ...
    methods: {
      ...
      setAdminTeam(checked){
        if(checked.length){
          this.ruleForm.jqxxKqyDepts = checked.map((r,i)=>{
            r.hasMain = !this.hasCddw&&i===0;
            return r;
          })
        }else {
      ...
      onSubmit() {
        ...
        form.jqxxKqyDepts = form.jqxxKqyDepts.map(r=>({...r,hasMain:r.hasMain?1:0}));
        kqyProJqxxAdd(form)
          .then(res => {
            if (res.code == "200") {
              this.ifSubmit = true;
      ...
      hasCddwHandle(value){
        if(!value) {
          this.ruleForm.sfzrqzz = 'N';
          if(this.ruleForm.jqxxKqyDepts.length) this.ruleForm.jqxxKqyDepts[0].hasMain = true;
        }else {
          this.ruleForm.jqxxKqyDepts = this.ruleForm.jqxxKqyDepts.map(r=>({...r,hasMain:false}));
        }
        this.ruleForm.setOff = [];
        this.showValue.setOff = "";
      },
      isZrqzz(value){
        if(!this.hasCddw) this.ruleForm.sfzrqzz = "N";
        else this.ruleForm.sfzrqzz = value;
      },
      //获取警情时间限制
      getJqsjxz(){
        init().then(r=>{
      ...
      //总队选择
      seleMain(index){
        this.ruleForm.jqxxKqyDepts = this.ruleForm.jqxxKqyDepts.map((r,i)=>{
          r.hasMain = i===index;
          return r;
        })
      }
