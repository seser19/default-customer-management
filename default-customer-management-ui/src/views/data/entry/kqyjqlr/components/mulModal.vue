  ...
  <el-dialog :visible.sync="mulModal" @close="closeDialogHandle">
    ...
    <div class="modal_tree" v-loading="loading">
      <el-row :gutter="20">
        <el-col :span="15">
          <h5>消防队名称</h5>
          <div class="tree_zone" v-if="!clickStatus && !searchResultList.length">
            <el-tree
              ref="tree"
              :props="props"
              show-checkbox
          ...
          <div class="name-list" v-else>
            <el-checkbox-group v-model="tags" v-if="searchResultList.length && clickStatus">
              <el-checkbox
                v-for="item in searchResultList"
                :key="item.deptId"
                :label="item"
                :disabled="item.deptLevel < 5"
                @change="chexboxChange"
  ...
  methods: {
    ...
    loadNode(node, resolve) {
      if (node.level === 0) {
        ...
        this.loading = true;
        checkKsDeptList(this.queryParams)
            .then(res => {
        ...
        checkKsDeptList({ parentId: node.data.deptId,deptLevel:node.data.deptLevel})
          .then(res => {
            resolve(this.handleData(res.data));
    ...
    checkClick(x) {
      let arrNodes = this.getLastNodes();
      if(x.deptLevel == 4 && x.hasChildren == false) {
        arrNodes = arrNodes.filter(b => {
           return b.deptLevel > 4;
        })
      }
      let arrNew = [...arrNodes,...this.tags];
      // this.tags = this.getLastNodes();
      this.tags = [...new Set(arrNew)];
      this.$refs.tree && this.$refs.tree.setCheckedNodes(this.tags);
    },
    // 获取末端节点数组
    getLastNodes() {
