        ...
        .hx-body
            ...
            el-form()
                ...
                el-row
                    ...
                    el-col(:span="7" v-if="activeName==='second'")
                        el-form-item(prop="")
                            ...
                            el-select(v-model="formData.czqkdm" placeholder="处置情况" clearable)
                                el-option(v-for="item in dicCzqk"
                                    :key="item.dictValue"
                                        :label="item.dictLabel"
                                        :value="item.dictValue")
                    el-col(:span="7" v-show="listData01.length>0 && activeName==='first' && formData.ksrq && formData.jsrq")
                        el-button(type="primary" @click="downloadResult()" :loading="downloadLoading") 导&nbsp;出
                    el-col(:span="25" v-show="listData02.length>0 && activeName==='second' && formData.ksrq && formData.jsrq")
                        el-button(style="margin-top: 30%;" type="primary" @click="downloadResult()" :loading="downloadLoading") 导&nbsp;出
            el-tabs(v-model="activeName" type="card" style="width:100%")
                el-tab-pane(label="出动填报超时" name="first")
                    el-table(:data="listData01" style="width: 100%" v-loading="listDataLoading01" @sort-change="tableSortChangeHandle" :default-sort="{prop: 'tbsj', order: 'descending'}" :empty-text="querydata01 == true ? '点击按钮查询':'暂无数据'")
                        el-table-column(prop="cdbh" label="出动编号" width="150" show-overflow-tooltip sortable="custom")
                ...
                el-tab-pane(label="出动审核超时" name="second")
                    el-table(:data="listData02" style="width: 100%" v-loading="listDataLoading02" @sort-change="tableSortChangeHandle" :default-sort="{prop: 'tbsj', order: 'descending'}" :empty-text="querydata02 == true ? '点击按钮查询':'暂无数据'")
                        el-table-column(prop="cdbh" label="出动编号" width="150" show-overflow-tooltip sortable="custom")
                        el-table-column(prop="jqbh" label="警情编号" width="150" show-overflow-tooltip sortable="custom")
                        el-table-column(prop="jjsj" label="接警时间" width="150" sortable="custom")
                            template(slot-scope="scope") {{scope.row.jjsj || '-'}}
...
<script>
    ...
    import {getJqcdlb,deriveFile,cdxxAuditByTimeDownload} from "@/api/data/entry/jqxx";
    ...
    import { getCustomReportRestrictValue } from "../../../api/report/report";
    ...
    export default {
        ...
        data() {
            return {
                ...
                listData01: [],
                listData02: [],
                listDataLoading01: false,
                listDataLoading02: false,
            ...
            downloadResult(){
                ...
                if(this.activeName==='first'){
                    deriveFile(this.formData).then(response => {
                        ...
                        downloadFile(`出动填报超时(${this.formData.ksrq}至${this.formData.jsrq})`, response.data.stream);
                    }).finally(() => {
                        this.downloadLoading = false
                    });
                }else if(this.activeName==='second'){
                    cdxxAuditByTimeDownload(this.formData).then(response => {
                        console.log(response);
                        downloadFile(`出动审核超时(${this.formData.ksrq}至${this.formData.jsrq})`, response.data.stream);
                    }).finally(() => {
                        this.downloadLoading = false
                    });
                }
                        ...
                        .then(res => {
                            if (res.code == 200) {
                                if (!res.data.hasresult) {
                                    this.listData01 = [];
                                    this.queryParams.total01 = 0;
                                    if(this.listData01.length == 0){
                                        this.querydata01 = false;
                                    }
                                    return;
                                }
                                // console.log(res.data.table.rows)
                                this.listData01 = res.data.table.rows;
                                this.queryParams.total01 = res.data.table.total;
                            }
                        })
                        .finally(() => {
                            $this.listDataLoading01 = false;
                        });
                }else if(this.activeName==='second'){
                    $this.listDataLoading02 = true;
                    // 组装数据
                    ...
                    historyCdxx
                        ...
                        .then(res => {
                            if (res.code == 200) {
                                if (!res.data.hasresult) {
                                    this.listData02 = [];
                                    this.queryParams.total02 = 0;
                                    if(this.listData02.length == 0){
                                    this.querydata02 = false;
                                }
                                    return;
                                }
                                // console.log(res.data.table.rows)
                                this.listData02 = res.data.table.rows;
                                this.queryParams.total02 = res.data.table.total;
                            }
                        })
                        .finally(() => {
                            $this.listDataLoading02 = false;
                        });
                }
            },
            openProvModal() {
