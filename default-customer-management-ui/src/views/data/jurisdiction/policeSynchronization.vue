        ...
        <div v-show="!show && !hzShow" class="page_zone">
            ...
            <div class="content">
                <div class="search_zone">
                    ...
                    <el-form ref="searchForm" :model="form">
                        <el-row>
                            ...
                            <el-col :span="7">
                                <el-form-item>
                                    <el-select v-model="form.jqdj" placeholder="警情等级" clearable>
                                        <el-option
                                            v-for="item in jqdj"
                                            :key="item.dictValue"
                ...
                <el-tabs v-model="activeName" type="card">
                    ...
                    <div class="table_zone">
                      ...
                      >
                          ...
                          <!-- <el-table-column
                              ...
                              width="150"
                          /> -->
                          <el-table-column prop="xzqymc" label="行政区域" :show-overflow-tooltip="true" min-width="9%"/>
                          <el-table-column prop="jqfsdd" label="警情地址" :show-overflow-tooltip="true" min-width="13%"/>
                          <el-table-column prop="sjlbmc" label="警情类型" :show-overflow-tooltip="true" min-width="13%"/>
                          <!-- <el-table-column
                              prop="sjlbmc"
                              label="警情类型"
                              sortable="custom"
                          ...
                          <el-table-column label="操作" min-width="13%">
                              <template slot-scope="scope">
                                  ...
                                  <el-button type="text" size="mini" @click="editDetail(scope.row)" v-hasPermi="['data:entry:jjxx-jcj:send']">派发</el-button>
                                  <el-button type="text" size="mini" @click="hangUp(scope.row)" v-hasPermi="['data:entry:jjxx-jcj:suspend']">挂起</el-button>
                                  <el-button
                                      type="text"
                                      size="mini"
                                      @click="handleDelete(scope.row)"
                                      v-hasPermi="['data:entry:jjxx-jcj:end']"
                                  >终止
                                  </el-button>
                              </template>
                          </el-table-column>
                  ...
                  <el-tab-pane label="已挂起" name="second">
                    <div class="table_zone">
                      <el-table
                          v-loading="loading"
                          :data="tableData2"
                      ...
                      >
                          ...
                          <el-table-column label="操作" min-width="13%">
                              <template slot-scope="scope">
                                  ...
                                  <el-button type="text" size="mini" @click="detailShow(scope.row)">查看</el-button>
                              </template>
                          </el-table-column>
                      </el-table>
            ...
            <el-dialog title="挂起信息" :visible.sync="dialogFormVisible1" :before-close="cancelDialogFormVisible1">
              <el-form :model="form">
                <el-form-item label="挂起原因" prop="region">
                  <el-select v-model="formFirst.region" placeholder="请选择挂起原因">
                    <el-option label="未归队" value="1"></el-option>
                    <el-option label="结案中" value="2"></el-option>
                    <el-option label="已派发" value="3"></el-option>
                    <el-option label="信息有误" value="4"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="补充说明" prop="explain">
                  <el-input v-model="formFirst.explain" autocomplete="off" type="textarea"
                ...
                <el-form-item label="附件说明">
                  <el-upload
                    ...
                    <div slot="tip" class="el-upload__tip">只能上传jpg/png/jpeg/gif等图片</div>
                  </el-upload>
                </el-form-item>
              </el-form>
              <div slot="footer" class="dialog-footer">
                <el-button @click="cancelDialogFormVisible1">取 消</el-button>
                <el-button type="primary" @click="sureDialogFormVisible1">确 定</el-button>
              </div>
            </el-dialog>
            <el-dialog title="终止信息" :visible.sync="dialogFormVisible2" :before-close="cancelDialogFormVisible2">
              <el-form :model="form">
                <el-form-item label="终止原因" prop="region">
                  <el-select v-model="formSecond.region" placeholder="请选择终止原因">
                    <el-option label="测试警" value="1"></el-option>
                    <el-option label="非消防事务警" value="2"></el-option>
                    <el-option label="已派发" value="3"></el-option>
                    <el-option label="其他" value="4"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="补充说明" prop="explain">
                  <el-input v-model="formSecond.explain" autocomplete="off" type="textarea"
                    :autosize="{ minRows: 2, maxRows: 4}"
                    style="width:400px"
                    placeholder="请输入内容"></el-input>
                </el-form-item>
              ...
              <div slot="footer" class="dialog-footer">
                <el-button @click="cancelDialogFormVisible2">取 消</el-button>
                <el-button type="primary" @click="sureDialogFormVisible2">确 定</el-button>
              </div>
            </el-dialog>
        </div>
...
<script>
    ...
    import { getOptions } from "@/api/data/entry/jqxx";
...
import { log } from '_@antv_g2plot@2.3.40@@antv/g2plot/lib/utils';
    ...
    export default {
        ...
        methods: {
            ...
            sureDialogFormVisible1(){
              if(!this.formFirst.explain){
                this.$message.error('补充说明不能为空')
                return
              }
              const { id } = this.ts1;
              this.$confirm(
                ...
                .then(() => {
                  ...
                  updateZtByIdBatch({
                    ids:[id+''],
                    zt:'3',
                    issueType:this.formFirst.region,
                    issueDesc:this.formFirst.explain
                  }
                  )
                    .then(res => {
                      res.code === 200 && this.msgSuccess(res.msg);
                      this.formFirst.region = '1'
                      this.formFirst.explain = ''
                      this.fileList1 = []
            ...
            uploadImage1 (params) {
              if (this.state1) {
                ...
                formData.append('eventId',this.ts1.id );
                formData.append('remark', ''); // 图片描述，当前传空，后期修改添加
                formData.append('type', '接处警挂起');
            ...
            beforeAvatarUpload1 (file) {
              ...
              if (!isJPG) {
                this.state1 = false;
                this.$message.error('上传图片只能是 JPG, JPEG, PNG, GIF,');
              }else{
                this.state1 = true;
              }
              console.log(isJPG);
            ...
            handleRemove1(file, fileList) {
              ...
              deletePicByAvater({ avatar: file.avatar }).then(res => {
                ...
                this.fileList1 = this.fileList1.filter(f => f.uid !== file.uid);
              });
            },
            handleExceed1(){
              this.$message({ message: '最多上传一份文件', type: 'info' });
            },
            hangUp(item){
            ...
            sureDialogFormVisible2(){
              if(!this.formSecond.explain){
                this.$message.error('补充说明不能为空')
                return
              }
              const { id } = this.ts2;
              this.$confirm(
                ...
                .then(() => {
                  ...
                  updateZtByIdBatch({
                    ids:[id+''],
                    zt:'-1',
                    issueType:this.formSecond.region,
                    issueDesc:this.formSecond.explain
                  }
                  )
                    .then(res => {
                      res.code === 200 && this.msgSuccess(res.msg);
                      this.formSecond.region = '1'
                      this.formSecond.explain = ''
                      this.fileList2 = []
            ...
            uploadImage2 (params) {
              if (this.state2) {
                const formData = new FormData();
                formData.append('file', params.file); // 上传的文件
                formData.append('eventId',this.ts2.id );
                formData.append('remark', ''); // 图片描述，当前传空，后期修改添加
                formData.append('type', '接处警删除');
                // formData.append('zqbh', this.form.zqbh); // zqbh火灾编号
                // 调用上传接口
                Upload(formData).then(res => {
                  this.fileList2.push({
            ...
            handleRemove2(file, fileList) {
              ...
              deletePicByAvater({ avatar: file.avatar }).then(res => {
                this.$message({ message: '删除成功', type: 'success' });
                this.fileList2 = this.fileList2.filter(f => f.uid !== file.uid);
              });
            },
            handleExceed2(){
              this.$message({ message: '最多上传一份文件', type: 'info' });
          ...
          handleDelete2(item) {
            ...
            // //       item.cdxxtb == "全部已完成" &&
            // //       item.zqxxtb == "已完成") ||
            // //     (item.sjlbmc !== "火灾扑救" && item.cdxxtb == "全部已完成")
            // //   ) {
            // //     this.$message.warning("审核通过的警情不能删除");
            // //     return;
            // //   }
            // // }
            // this.$confirm(
