<template lang="pug">
    .app-container
        ...
        .hx-body
            ...
            el-form()
                el-row
                    ...
                    el-col(:span="3")
                        el-button(type="primary" @click.prevent="multipleTurnDownHandle") 批量驳回
                    el-col(:span="7")
                        el-form-item(prop="")
                            el-select(v-model="formData.czqkdm" placeholder="处置情况" clearable)
                                el-option(v-for="item in dicCzqk"
                                    :key="item.dictValue"
                                        :label="item.dictLabel"
                                        :value="item.dictValue")
            el-table(ref="tableList" :data="listData" style="width: 100%" v-loading="listDataLoading" @sort-change="tableSortChangeHandle" :default-sort="{prop: 'tbsj', order: 'descending'}" @selection-change="handleSelectionChange" :empty-text="querydata == true ? '点击按钮查询':'暂无数据'")
                el-table-column(type="selection" width="50" :selectable="judgeRejectBoolean")
                el-table-column(label="出动编号" width="140" show-overflow-tooltip sortable="custom")
                    template(slot-scope="scope")
                        a(style="color: #1890ff;" @click="jumpXq(scope.row)") {{scope.row.cdbh}}
                el-table-column(label="警情编号" width="140" show-overflow-tooltip sortable="custom")
                    template(slot-scope="scope")
                        a(style="color: #1890ff;" @click="toJqDetail(scope.row)") {{scope.row.jqbh}}
                ...
                el-table-column( label="投入力量")
                    template(slot-scope="scope") {{scope.row.carCount||0}}车{{scope.row.trrys||0}}人
                el-table-column(prop="tbzhName" label="填报单位" width="140" show-overflow-tooltip)
                el-table-column(prop="tbsj" label="填报时间" width="160" sortable="custom" show-overflow-tooltip)
                ...
                el-table-column(prop="state" label="操作" width="100")
                    template(slot-scope="scope")
                        ...
                        //- 非 未送审、被驳回、归档、废弃
                        el-button(v-if="scope.row.shzt != '0' && scope.row.shzt != '4' && scope.row.shzt != '99' && scope.row.shzt != '-99'" type="text" @click="turnDownHandle(scope.row)") 驳回
        ...
        //- 驳回通用确认组件
        reject-confirm-dialog(ref="RejectConfirmDialog" @update-component-data="submitTurnData")
...
<script>
    ...
    import {cdReject} from "@/api/check/reject";
    import RejectConfirmDialog from "@/components/RejectConfirmDialog";
    import { formdate } from "@/utils/dateTimeUtils.js";
    // 主战
    ...
    //   },
      created() {
            // 新需求：注释掉 2020.2.25
            // this.formData.ksrq = getNearlyRecentDays(30)[0];
            // this.formData.jsrq = getNearlyRecentDays(30)[1];
            ...
            const endDate = timestamp.getFullYear() + '-' + (timestamp.getMonth() + 1).toString().padStart(2, '0') + '-' + timestamp.getDate().toString().padStart(2, '0');
            timestamp.setDate(timestamp.getDate() - 30);
            const startDate = timestamp.getFullYear() + '-' + (timestamp.getMonth() + 1).toString().padStart(2, '0') + '-' + timestamp.getDate().toString().padStart(2, '0');
            this.formData.ksrq = startDate;
            this.formData.jsrq = endDate;
        },
        methods: {
            clear(){
                ...
                let end = new Date();
                this.formData.ksrq = formdate(new Date(start)).substr(0,10);
                this.formData.jsrq = formdate(new Date(end)).substr(0,10);
                this.selectDate = 2;
            },
            //处置情况
            getDictsData() {
            ...
            turnDownHandle(row) {
                ...
                // 初始化组件数据
                this.$refs["RejectConfirmDialog"].cancelDialog();
                this.$refs["RejectConfirmDialog"].dialogVisible = true;
                this.turnDownCodes = row.cdbh;
            },
            /**
             * 批量数据驳回回调
             */
            multipleTurnDownHandle() {
                // 容错
                if (this.turnDownCodes) {
                    // 初始化组件数据
                    this.$refs["RejectConfirmDialog"].cancelDialog();
                    this.$refs["RejectConfirmDialog"].dialogVisible = true;
                }
            ...
            submitTurnData(data) {
                // 容错
                if (data && data.finishDate) {
                    const params = {...data};
                    params.cdbh = this.turnDownCodes;
                    cdReject(params)
                        .then(res => {
                            const {msg, code} = res;
                            if (code === 200) {
            ...
            handleSelectionChange(val) {
                if (val && val.length) {
                    let saveData = [];
                    val.forEach(element => {
                        saveData.push(element.cdbh);
                    });
                    this.turnDownCodes = saveData.join(",");
                } else {
                    this.turnDownCodes = "";
                }
            },
             ...
             * 格式化表格多选项，判断是否可以选中
             */
            judgeRejectBoolean(row, index) {
                const {shzt} = row;
                // 非 未送审、被驳回、归档、废弃
                if (shzt != "0" && shzt != "4" && shzt != "99" && shzt != "-99") {
                    return true;
                } else {
                    return false;
                }
