<style lang="scss">
    .page-custom-report-template-update {
        background: #f4f6f8;
        padding: 12px;
        .upper-half {
            padding: 4px 22px 0;
...
<template>
    <div class="page-custom-report-template-update" v-loading="submitting">
                    ...
                    <el-row :gutter="30">
                        ...
                        <el-col :span="8">
                            <el-form-item label="模板类型" prop="belongType">
                                <el-select v-model="form.belongType" disabled size="small">
                                    <el-option
                        ...
                        <div class="config row" :class="{close: !rowConfigOpen}" v-loading="rowOptionLoading">
                            ...
                            <div class="config-option">
                                <div class="option-tree">
                                    ...
                                    <div class="option-group" v-for="o in rowOption">
                        ...
                        <div class="config column" :class="{large: !rowConfigOpen}" v-loading="columnOptionLoading">
                            ...
                            <div class="config-option">
                                <div class="option-tree">
                                    ...
                                    <div class="option-group" v-for="o in columnOption">
                                        <h5 @click="toggleSubNode(o)">{{o.name}}<i class="el-icon-arrow-up" :class="{close: o.close}"></i></h5>
                                        <ul :id="'subNode_' + o.id" :style="{height: o.children.length * 28 + 'px'}">
                                            <li v-for="c in o.children">
                ...
                <el-col :span="17">
                    <div class="preview-section">
                        ...
                        <div class="sort-column">
                            <ul>
                                <li v-for="(column, index) in selectedColumn">
...
<script>
    import { getCustomReportTemplateDetail, getCustomReportRowConfig, getCustomReportColumnConfig, editCustomReportTemplate } from '@/api/report/report';
    ...
    export default {
        ...
        methods: {
            back() {
                const visitedRoutes = this.$store.state.tagsView.visitedViews;
                let currentRoute = {};
                for (const r of visitedRoutes) {
                    if (r.path.indexOf('/eqw/customReport/update/') > -1) {
                        currentRoute = r;
                        break;
                    }
                }
                this.$store.dispatch("tagsView/delView", currentRoute).then(({ visitedViews }) => {
        ...
        async mounted() {
            // 获取模板详情
            const detailResponse = await getCustomReportTemplateDetail({tempId: this.$route.params.id});
            if (detailResponse.code !== 200) {
                this.$message.error(detailResponse.msg);
                return;
            }
            const detail = detailResponse.data;
            this.form.tempCode = detail.tempCode || '';
            this.form.tempName = detail.tempName || '';
            this.form.belongType = detail.belongType || '1';
            this.form.tempScope = detail.tempScope || '0';
            this.form.crossTitle = detail.crossTitle || '';
            this.form.tempRemark = detail.tempRemark || '';
            this.useTotal = detail.colTotal === '0';
            // 获取可配置行并回填已选行
            this.rowOptionLoading = true;
            const rowResponse = await getCustomReportRowConfig({belongType: this.form.belongType});
            this.rowOptionLoading = false;
            if (rowResponse.code !== 200) {
                this.$message.error(rowResponse.msg);
                return;
            }
            ...
            // 获取可配置列并回填已选列
            this.columnOptionLoading = true;
            const columnResponse = await getCustomReportColumnConfig({belongType: this.form.belongType, rowConfId: selectedRow.id});
            this.columnOptionLoading = false;
            if (columnResponse.code !== 200) {
                this.$message.error(columnResponse.msg);
                return;
            }
            const columns = columnResponse.data || [];
            columns.map(column => {
                column.close = false;
                column.children.map(c => {
                    c.checked = false;
                    detail.column.forEach((co, i, arr) => {
                        if (co.confId === c.id) {
                            c.checked = true;
                            this.selectedColumn[co.instSort] = c;
                        }
                    })
                });
            });
