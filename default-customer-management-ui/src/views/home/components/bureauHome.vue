...
<template>
    <div class="indexPage">
        ...
        <el-row class="content section panel_group" v-loading="warningListDataLoading">
            <el-col class="panel_item" v-for="item in warningListData" :key="item.title" :xs="12" :sm="8" :md="6" :lg="4" :xl="4" :span="4">
                <div class="label">
                    <img :src="item.img" />
                    <h5>{{ item.title }}</h5>
                </div>
            ...
            <el-col :xs="24" :sm="24" :lg="14" :span="14" v-loading="mapListDataLoading">
                <el-row class="section map-sec">
                    <el-col :xs="24" :sm="17" :lg="17" :span="17">
                        <div class="map_zone">
                            <v-chart v-if="mapListData.length" ref="mapChart" :options="chartsOption.map" autoresize :manual-update="true" @click="mapClickHandle" />
                            <div v-else class="data-none">暂无数据</div>
                            <el-button class="show-china-map" v-if="deptLevel === 1" plain @click="mapClickHandle({ data: mapsSaveChinaData }, 'china')">返回全国</el-button>
                        </div>
                    </el-col>
                    <el-col :xs="24" :sm="7" :lg="7" :span="7">
                        <div class="chart_title">火灾分布</div>
                        <div v-if="mapListData.length" class="swipe_zone">
                            <el-carousel trigger="click" height="300px" arrow="never" indicator-position="outside" :loop="false" :autoplay="false">
                                <el-carousel-item v-for="(item,index) in mapListData" :key="index">
                                    <div class="carousel-content" v-for="progress in item" :key="progress.areaId">
                                        ...
                                        <div class="carousel-line">
                                            <div class="line-active" :style="{width: (progress.hzqsCnt / chartsOption.map.visualMap.max) * 100 + '%' }"></div>
                                        </div>
                    ...
                    <div class="rank-list">
                        <ul>
                            <li v-for="(r, index) in rankList" :key="r.count">
                                <span class="rank-num" :class="{'medal-gold': index === 0, 'medal-silver': index === 1, 'medal-bronze': index === 2}">{{ index > 2 ? index + 1 : '' }}</span>
...
<script>
    ...
    // 省市字典
    import { getProvincesMap, getProvinceAbbr, getProvinceName, getCityMap, getMapJSONData } from "@/utils/mapDictionary";
    ...
    export default {
                    ...
                    line: {
                        ...
                        tooltip: {
                            ...
                            formatter: params => {
                                let tooltipContent = '';
                                // 全国视图时获取二级地区全名
                                if (this.pagePostData.areaLevel === 1) {
                                    tooltipContent += getProvinceName(params[0].name) + '<br>';
                                } else {
                                ...
                                if (params.length > 1) {
                                    ...
                                    tooltipContent += this.isTb + '：' + + this.lineListData[this.currentBarData]['同比'][params[0].dataIndex] + '%';
                                }
        ...
        methods: {
            ...
            async getPageData(data) {
                ...
                if (data) {
                    me.pagePostData.areaId = data.areaId;
                    me.pagePostData.areaLevel = data.areaLevel;
            ...
            getBarChartDatas() {
                ...
                corpsHome.getBarChartDatas(params).then(res => {
                    ...
                    if (code === 200 && data && data.length) {
                        ...
                        data.forEach(element => {
                            if (me.pagePostData.areaLevel === 1) {
                                me.chartsOption.line.xAxis.data.push(getProvinceAbbr(element.areaId));
                            } else {
                        ...
                        if (this.chartsOption.line.xAxis.data.length < 21) {
                            ...
                            this.chartsOption.line.series.map(barConfig => {
                                ...
                                barConfig.itemStyle.normal.barBorderRadius = 7;
                            });
                        } else {
                            if (this.pagePostData.areaLevel !== 1) {
                            ...
                            this.chartsOption.line.series.map(barConfig => {
                                ...
                                barConfig.itemStyle.normal.barBorderRadius = 3;
                            });
                        }
                        // 设置默认值
